...
async function commitStaged() {
  ...
  await quiet(withBusy(state.amend ? "Amending…" : "Committing…", async () => {
    const result = await act("commit", { message, amend: state.amend });
    toast("ok", result.message);
    state.commitMessage = "";
    state.amend = false;
    closeViewer();
...
async function cloneRepository() {
  ...
  await quiet(withBusy(`Cloning ${answer.value}…`, async () => {
    const result = await call("clone_repo", { url: answer.value, parent: answer.extra, name: "" });
    toast("ok", result.message);
    state.selected = null;
    state.view = null;
    await refresh(false);
...
async function switchBranch(name) {
  await quiet(withBusy(`Switching to ${name}…`, async () => {
    const result = await act("checkout", { branch: name });
    toast("ok", result.message);
    await refresh(false);
  }));
}
...
async function applyHunk(patch, target, said = "") {
  ...
  await quiet(withBusy(said ? "Applying…" : "Applying the hunk…", async () => {
    const result = await act(`${target}-hunk`, { patch });
