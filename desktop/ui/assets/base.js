...
// Every call reaches the engine through Tauri's IPC: no server, no port, no URL.
async function call(command, args = {}) {
  const core = (window.__TAURI__ && window.__TAURI__.core) || null;
  if (!core) throw new Error("This page only runs inside the GitSquid window.");
  try {
    return await core.invoke(command, args);
  } catch (failure) {
    // A command answers with Err(String), and Tauri rejects with that string itself.
    throw new Error(typeof failure === "string" ? failure : failure?.message || "The command failed.");
  }
}

// The one action command, keyed by name; the payload carries only the fields that action reads.
const act = (action, payload) => call("worktree_action", { action, payload: payload || {} });
