...
function commitContext(commit) {
  return {
    ...
    rev: commit.sha,
    files: commit.files,
    fetch: (path) => call("commit_patch", { sha: commit.sha, path, ...readingFlags() }),
  };
}
...
function worktreeContext(staged) {
  ...
  return {
    ...
    files,
    fetch: (path) => call("file_diff", { path, staged, ...readingFlags() }),
...
const readingFlags = () => ({ ws: state.diffView.space, ctx: state.diffView.context });
...
async function openBlame(context, path) {
  ...
  renderViewer();
  try {
    const payload = await call("blame", { path, rev: context.rev || "" });
